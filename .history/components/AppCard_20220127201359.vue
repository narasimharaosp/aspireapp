      ...
      <b-carousel-slide v-for="card in cards" :key="card.lastNumber" img-blank :img-alt="card.name" :class="card.type">
        <div class="container card-info">
          ...
          <div class="row card-number">
            <div v-if="showCard" class="col">
              <div v-for="i in 3" :key="i" class="four-dots">
                <span v-for="index in 4" :key="index"/>
              </div>
              <h2>{{ card.lastNumber }}</h2>
            </div>
            <div v-else-if="card.fullNumber" class="col">
              <div class="full-number">{{ card.fullNumber[0] }}</div>
              <div class="full-number">{{ card.fullNumber[1] }}</div>
              <div class="full-number">{{ card.fullNumber[2] }}</div>
              <div class="full-number">{{ card.fullNumber[3] }}</div>
            </div>
          </div>
...
<script>
  export default {
    ...
    methods: {
      getNumberByIndex(card, i) {
        if (!card.fullNumber) {
          return ''
        }
        return card.fullNumber.match(/.{1,3}/g)[i]
      }
    }
  }
</script>
